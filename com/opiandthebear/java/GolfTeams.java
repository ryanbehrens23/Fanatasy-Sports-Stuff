...
public class FantasyGenerator {
    ...
    private final static String[] teamA = new String[]{""};
    private final static String[] teamB = new String[]{""};
    private final static String[] teamC = new String[]{""};
    private final static String[] teamD = new String[]{""};
    public static void main(String[] args){
        //Creates teamList list from the teams listed teams String Array
...
//        System.out.println("Miller Division: \n" + teamsList.subList(6, 12));
    ...
    public static void getH2HDoubles() {
        Collections.shuffle(Arrays.asList(teamA));
        Collections.shuffle(Arrays.asList(teamB));
        Collections.shuffle(Arrays.asList(teamC));
        Collections.shuffle(Arrays.asList(teamD));
        Boolean[] teamAChosen = new Boolean[teamA.length];
        Boolean[] teamBChosen = new Boolean[teamB.length];
        Boolean[] teamCChosen = new Boolean[teamC.length];
        Boolean[] teamDChosen = new Boolean[teamD.length];
        Arrays.fill(teamAChosen, false);
        Arrays.fill(teamBChosen, false);
        Arrays.fill(teamCChosen, false);
        Arrays.fill(teamDChosen, false);
        int position = 0;
        while (true) {
            Random rand = new Random();
            int randomInt1 = rand.nextInt(teamA.length);
            int randomInt2 = rand.nextInt(teamB.length);
            int randomInt3 = rand.nextInt(teamC.length);
            int randomInt4 = rand.nextInt(teamD.length);
            // get random person
            String person1 = teamA[randomInt1];
            String person2 = teamB[randomInt2];
            String person3 = teamC[randomInt3];
            String person4 = teamD[randomInt4];
            for (int week = 1; week <= 6; week++) {
                if (!person1.equals(person1) && !person2.equals(person2) && !person3.equals(person3) && !person4.equals(person4) && teamAChosen[randomInt1] == false && teamBChosen[randomInt2] == false && teamCChosen[randomInt3] == false && teamDChosen[randomInt4] == false) {
                    if (!person1.equals(teamA[position]) && !person1.equals(teamB[position]) && !person1.equals(teamC[position]) && !person1.equals(teamD[position])) {
                        ...
                        teamAChosen[randomInt1] = true;
                        teamBChosen[randomInt2] = true;
                        teamCChosen[randomInt3] = true;
                        teamDChosen[randomInt4] = true;
                    }
                    if (isArrCompleted(teamAChosen) && isArrCompleted(teamBChosen) && isArrCompleted(teamCChosen) && isArrCompleted(teamDChosen))
                        break;
                }
            }
        }
    }
    public static void getH2HTeams() {
    ...
    public String[] printDraftOrder(String[] team) {
